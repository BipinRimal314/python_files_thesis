...
class InsiderLabeler:
    ...
    def label_simple(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # If using pseudonymized data, we need to check original user mapping
        # For now, try to match directly
        for insider_user in self.insider_users:
            if user_col == 'user_pseudo':
                # Need to pseudonymize the insider username
                pseudo_user = utils.pseudonymize_identifier(insider_user)
                mask = df[user_col] == pseudo_user
            else:
                mask = df[user_col] == insider_user
            
            n_marked = mask.sum()
            if n_marked > 0:
                df.loc[mask, 'is_insider'] = 1
                logger.info(f"  Labeled {n_marked:,} records for insider {insider_user}")
        ...
        logger.info(f"  Insider records:  {total_insider:,} ({total_insider/len(df)*100:.2f}%)")
    ...
    def label_temporal(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for period in self.insider_periods:
            ...
            if n_marked > 0:
                ...
                logger.info(
                    f"  {insider_user}: {n_marked:,} records "
    ...
    def label_processed_data(self, method: str = 'temporal'):
        ...
        # Apply labeling
        if method == 'temporal':
            df = self.label_temporal(df)
        else:
            df = self.label_simple(df)
    ...
    def create_summary_report(self, df: pd.DataFrame):
        ...
        logger.info(f"  Insider (1):      {insider_count:,} ({insider_count/total*100:.2f}%)")
        logger.info(f"  Imbalance ratio:  1:{normal_count/max(insider_count, 1):.0f}")
        ...
        with open(summary_file, 'w') as f:
            ...
            f.write(f"Insider: {insider_count:,} ({insider_count/total*100:.2f}%)\n")
            f.write(f"\nInsider Users:\n")
            for user in sorted(self.insider_users):
                f.write(f"  - {user}\n")
