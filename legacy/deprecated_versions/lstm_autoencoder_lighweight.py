...
def main():
    ...
    try:
        ...
        logger.warning("Could not load user_ids for sequences. Predictions will not be user-mapped.")
        user_ids = ['unknown'] * len(y_test)

    # Need to align user_ids with X_test (this is complex, simplified for now)
    # This assumes X_test == X, which it does in our new design.
    if len(user_ids) != len(y_test):
         logger.warning(f"User IDs ({len(user_ids)}) and y_test ({len(y_test)}) mismatch. Predictions will not be user-mapped.")
         user_ids = ['unknown'] * len(y_test)
    ...
    results_df = pd.DataFrame({
        'user': user_ids, # Assuming user_ids align with X_test (which they should)
        'true_label': y_test,
